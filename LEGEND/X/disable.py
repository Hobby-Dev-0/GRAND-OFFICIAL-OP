...
from LEGEND.X.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from LEGEND.X.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from LEGEND.X.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "LEGEND.X." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "LEGEND.X." + args[0].rsplit(".", 1)[0]
